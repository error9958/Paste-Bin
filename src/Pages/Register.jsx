...
import { SignUp } from "../Scripts/UtilityMethods";
...
function Register() {
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    SignUp(userName, email, password, flag).then(() => {
      navigate("/", { replace: true });
    });
  };
  return (
    <div className=" h-screen bg-whitesmoke flex items-center  shadow-2xl text-sm ">
      <form
        onSubmit={(e) => {
          submitHandler(e);
        }}
        className="h-auto  md:w-3/4 lg:w-1/3 px-12 py-6  md:px-20  md:py-12  gap-4 flex flex-col items-start bg-white text-blkv mx-auto"
      >
        ...
        <button
          type="submit"
          className="bg-blkv hover:bg-green-500 text-white font-semibold  px-8 py-4"
        >
          SIGN UP
        </button>
      </form>
